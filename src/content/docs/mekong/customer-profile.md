---
title: Customer Profile
description: Use /customer-profile to create detailed customer personas
section: docs
---

# /customer-profile - Customer Persona Builder

Create detailed **Customer Profiles** for your products or services, optimized for your local market.

## Usage

```bash
/customer-profile "premium coffee shop"
/customer-profile "B2B software service"
/customer-profile "local organic grocery store"
```

## Agents Spawned

- **Researcher** - Market research and demographics analysis
- **Copywriter** - Key messages and value proposition

## Output Includes

| Section | Content |
|---------|---------|
| **Demographics** | Age, gender, location, income |
| **Behavior** | Purchase channels, frequency, AOV |
| **Pain Points** | Problems customers face |
| **Solutions** | Value proposition, key messages |

## Example Output

```markdown
## Customer Profile: Premium Coffee Shop

### 👥 Demographics
- Age: 25-45
- Gender: 60% Female, 40% Male
- Location: Urban professionals

### 😰 Pain Points
1. Tired of low-quality chain coffee
2. Need productive work environment
3. Want premium experience at fair price

### 💡 Key Message
> "Craft coffee, crafted for creatives"
```

## Use Cases

### For Startups
- Validate product-market fit
- Guide marketing strategy
- Optimize messaging

### For Businesses
- Segment customers
- Develop new products
- Improve customer experience

### For Agencies
- Pitch to clients
- Research for campaigns
- Content strategy

## Best Practices

1. **Be specific** - More detailed input = more accurate output
2. **Validate** - Cross-check with real customer data
3. **Update** - Review and refresh quarterly

---

## 🏯 Binh Pháp Alignment

> **地形篇** (Địa Hình) - Terrain - Know your customer

### Zero-Effort Commands

| Gõ lệnh | Auto-execute |
|---------|--------------|
| `/plan` | Tự tạo plan |
| `/marketing` | Tự phân tích market |
| `/scout` | Tự thu thập intel |

📖 [Xem tất cả Commands](/docs/commands)
